    ...
    <div class="container">
        ...
        <ul class="list-group">
            <%
                ...
                try {
                             ArrayList<Order> orders = orderDao.getUserOrder(currentUser.getId());
                             for (Order order : orders) {
            %>
                        <li class="list-group-item">
                        Numero de pedido: <%= order.getId() %></a> Fecha: <%= order.getDate() %> Plato:<%= order.getDishName() %> Precio:<%= order.getDishPrice() %>€
                        <%
                        if (!order.isPaid()) {
                        %>
